...
class SFCN():
    ...
    def train(self, ):
        ...
        train_data = dataloader.Market_Triplet_Loader(root=self.cfg.TRAIN.ROOT,
                                                      ...
                                                      repeat=3,
                                                      type='train')
        train_loader = Data.DataLoader(train_data, batch_size=self.cfg.TRAIN.K * self.cfg.TRAIN.P, shuffle=False,
                                       num_workers=8, drop_last=True)
        gallery_data = dataloader.Market_Loader(root=self.cfg.TEST.GALLERY,
                                                size=self.cfg.TRAIN.SIZE,
                                                type='gallery')
        gallery_loader = Data.DataLoader(gallery_data, batch_size=self.cfg.TEST.BATCH_SIZE, shuffle=False,
                                         num_workers=8, drop_last=False)
        query_data = dataloader.Market_Loader(root=self.cfg.TEST.QUERY,
                                              size=self.cfg.TRAIN.SIZE,
                                              type='query')
        query_loader = Data.DataLoader(query_data, batch_size=self.cfg.TEST.BATCH_SIZE, shuffle=False, num_workers=8,
                                       drop_last=True)
        # model_dict = self.model.state_dict()
        # pretrained_dict = torch.load('/media/yun/work/nx/code/MuDeep_v2-master_6/model/market/market_30.pkl',
        #                              map_location=self.device)
        # # pretrained_dict = {k:v for k,v in pretrained_dict.items()['state_dict'] if np.shape(model_dict[k])==np.shape(v)}
        ...
        for epoch in range(1, cfg.TRAIN.MAX_EPOCH + 1):
            ...
            for step, data in enumerate(train_loader):
                ...
                loss_Tri1 = torch.mean(
                    torch.clamp((positive1 - negative1 + cfg.TRAIN.MARGIN), min=0.0))  # TripHard Loss
                loss_Tri2 = torch.mean(
                    torch.clamp((positive2 - negative2 + cfg.TRAIN.MARGIN), min=0.0))  # TripHard Loss
                loss_Tri3 = torch.mean(
                    torch.clamp((positive3 - negative3 + cfg.TRAIN.MARGIN), min=0.0))  # TripHard Loss
                loss_Tri = loss_Tri1 + loss_Tri2 + loss_Tri3
                loss = 3 * loss_CEL + 1* loss_Tri

                # #############################
                # (4) Display and Backward
                # #############################
                epoch_loss += loss.item()
                # summary(self.model, input_size=(3, 384, 384), batch_size=128)
                print(
                    'Epoch: {}/{}  |  Step: {}/{}  |  lr: {:.6f}  |  loss_CEL: {:.6f}  |  loss_Tri: {:.6f}  |  Time: {:.3f}'
                    .format(epoch, self.cfg.TRAIN.MAX_EPOCH, step + 1, len(train_loader),
            ...
            if epoch % self.cfg.TRAIN.SNAPSHOT == 0:
                ...
                with torch.no_grad():
                    ...
                    print('-- Epoch:%d, Rank@1: %f, Rank@5: %f, Rank@10: %f, mAP: %f' % (
    ...
    def test(self, model_path, out_name, overwrite=False):
        ...
        with torch.no_grad():
            for path in paths:
                ...
                epoch = int(os.path.splitext(path)[0].split('-')[-1].split('_')[-1])
                ...
                print(
                    '-- Epoch:%d, Rank@1: %f, Rank@5: %f, Rank@10: %f, mAP: %f' % (epoch, cmc[0], cmc[4], cmc[9], map))
    ...
    def evaluate(self, gallery, query):
        ...
        for i in range(len(query_label)):
            ap_tmp, CMC_tmp = self._evaluate(query_feature[i], query_label[i], query_cam[i], gallery_feature,
    ...
    def build_model(self, ):
        self.model = network.SFCN(num_class=cfg.NUM_CLASS, num_scale=3, pretrain=True)
        self.model.train()
        if self.use_cuda:
    ...
    def build_optimizer(self, ):
        ignored_params = list(map(id, self.model.module.scale1.parameters())) + list(
            map(id, self.model.module.scale1_1.parameters())) + list(
            map(id, self.model.module.scale1_2.parameters())) + list(map(id, self.model.module.scale1_3.parameters())) + \
                         list(map(id, self.model.module.scale2.parameters())) + list(
            map(id, self.model.module.scale2_1.parameters())) + list(
            map(id, self.model.module.scale2_2.parameters())) + list(map(id, self.model.module.scale2_3.parameters())) + \
                         list(map(id, self.model.module.scale3.parameters())) + list(
            map(id, self.model.module.scale3_1.parameters())) + list(
            map(id, self.model.module.scale3_2.parameters())) + list(map(id, self.model.module.scale3_3.parameters())) + \
                         list(map(id, self.model.module.atten1.parameters())) + list(
            map(id, self.model.module.atten2.parameters())) + list(map(id, self.model.module.atten3.parameters()))
...
if __name__ == '__main__':
    ...
    engine.train()
    # engine.test(model_path='/media/yun/work/nx/code/MuDeep_v2-master_6/model/market/market_150.pkl',
